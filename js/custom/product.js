$(document).ready(function() {
  ...
  $(document).delegate('#add_product', 'click', function(event) {
      ...
      if (product_img == "" || full_name == "" || units_id == "" || category_id == "" || sales_price == "" || stock_quantity == "") {
          $('#reqProd').html("<div class='alert alert-danger'>All fields are required</div>");
          return;
      }

      // Validate numerical values
      var salesPriceIsNaN = isNaN(sales_price);
      var stockQuantityIsNaN = isNaN(stock_quantity);
      
      if (salesPriceIsNaN && stockQuantityIsNaN) {
          $('#reqProd').html("<div class='alert alert-danger'>Sales Price and Stock Quantity must be numerical values</div>");
          return;
      } else if (salesPriceIsNaN) {
          $('#reqProd').html("<div class='alert alert-danger'>Sales Price must be a numerical value</div>");
          return;
      } else if (stockQuantityIsNaN) {
          $('#reqProd').html("<div class='alert alert-danger'>Stock Quantity must be a numerical value</div>");
          return;
      }
  ...
  $.getJSON(api+'read_product.php', function(json) {
      ...
      for (var i = 0; i < json.length; i++) {
          ...
          obj['product_id'] = json[i].product_id;
          obj['cat_name'] = json[i].cat_name;
          obj['product_img'] = json[i].product_img;
  ...
  $.getJSON(api+'read_productInventory.php', function(json2) {
      ...
      for (var c = 0; c < json2.length; c++) {
          ...
          tr.push('<tr>');
          tr.push('<td>' + json2[c].product_id  + '</td>');
  ...
  $(document).delegate('#edit_product', 'click', function(event) {
      ...
      if(full_name == "") {
          $('#reqPdit').html("<div class='alert alert-danger'>Product Name is required</div>");
          return;
      }
      ...
      if (salesPriceIsNaN && stockQuantityIsNaN) {
          $('#reqPdit').html("<div class='alert alert-danger'>Sales Price and Stock Quantity must be numerical values</div>");
          return;
      } else if (salesPriceIsNaN) {
          $('#reqPdit').html("<div class='alert alert-danger'>Sales Price must be a numerical value</div>");
          return;
      } else if (stockQuantityIsNaN) {
          $('#reqPdit').html("<div class='alert alert-danger'>Stock Quantity must be a numerical value</div>");
          return;
      }
      ...
      function getdelID(product_id) {
          $.ajax({
              ...
              success: function(response2) {
                  ...
                  $('#del_fullname').val(response2.full_name);
              }
          });
      }
  });
